...
class pcie_rc_rx_pkt_checker:#(pcie_seq_rc_config_pkt):
    def rc_rx_checker():
        ...
        bad_pkts =0
        ep_gen_bad_pkts= 0
        ep_cfg_read_req_pkts_rcvd = 0
        ep_cfg_write_req_pkts_rcvd = 0
        ep_mem_read_req_rcvd = 0
        ep_mem_write_req_rcvd = 0
        ep_pkts_iter = 0
        ep_err_pkts = []
        compl_err_pkts = []
        for line in bin_f: 
        ...
        if compl_pkt_queue.qsize() > 0:
            ...
            #for i in range(compl_pkt_queue.qsize()):       
                ...
                if str_type == '01010':
                                                                                            ...
                                                                                            if str_tag in tag_queue.queue:                   # ep completer: tag will be within range 0 to 2**8-1
                                                                                                if int(str_reserved_r,2) == 0:           # ep completer: reserved_bit will be 0 
                                                                                                    ...
                                                                                                    if (int(str_lower_address             ,2)>0) in range(0,2**7-1): # ep completer: lower_address will be within 0 to 2**7-1
                                                                                                        ...
                                                                                                        compl_err_pkts.append(iteration)
                                                                                                    ...
                                                                                                    if int(str_ext_register_num      ,2) >= 0 :                                # ep completer: length will be 1
                                                                                                        if int(str_register_num      ,2) >= 0 :                                # ep completer: length will be 1
                                                                                                            if int(str_rsv_11_1      ,2) == 0 :                                # ep completer: length will be 1
                                                                                                                good_pkts = good_pkts + 1
                                                                                                                ep_gen_good_pkts=ep_gen_good_pkts+1                                                                                                
                                                                                                                ...
                                                                                                                rc_checker_f.write("\n")
                                                                                                                bad_pkts=bad_pkts+1                                                                                                
                                                                                                                ep_gen_bad_pkts=ep_gen_bad_pkts+1                                                                                                
                                                                                                                ep_err_pkts.append(ep_pkts_iter)
                    ...
                    rc_checker_f.write("\n") 
                    rc_checker_f.write("\n")                    
                    rc_checker_f.write("GOOD_PKTS RECIEVED = {}\n".format(str(good_pkts)))
                    rc_checker_f.write("BAD_PKT RECIEVED = {}\n".format(str(bad_pkts)))
                    rc_checker_f.write("ep_cfg_read_req_pkts_rcvd = {}\n".format(str(ep_cfg_read_req_pkts_rcvd)))
                    rc_checker_f.write("ep_cfg_write_req_pkts_rcvd = {}\n".format(str(ep_cfg_write_req_pkts_rcvd)))
                    rc_checker_f.write("ep_mem_read_req_rcvd = {}\n".format(str(ep_mem_read_req_rcvd)))
                    rc_checker_f.write("ep_mem_write_req_rcvd = {}\n".format(str(ep_mem_write_req_rcvd)))
                    rc_checker_f.write("total good packets from EP generator side = {}\n".format(str(ep_gen_good_pkts)))
                    rc_checker_f.write("total error/bad packets from EP generator side = {}\n".format(str(ep_gen_bad_pkts)))
                    rc_checker_f.write("\n")
                    rc_checker_f.write("\n")
                    rc_checker_f.write("\n")
                    ep_pkts_iter = ep_pkts_iter+1
...
# Sort i#n ascending order
        ...
        #    if ep_err_pkts_with_compl[j] == ep_err_arr_sorted[j]:
