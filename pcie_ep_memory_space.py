...
MEMORY_SIZE = 64 * 32  # 256B - Total number of memory locations is 64
...
def write_data_to_memory(start_address, data_rec):
    num_bits = len(bin(data_rec)[2:])  # Get the number of bits in the data
    num_chunks = (num_bits + 31) // 32

    for i in range(num_chunks):
        if start_address + i > Address_RANGE_END:
            break
        chunk = get_data_chunk(data_rec, i * 32, 32)
        memory[start_address + i] = chunk
